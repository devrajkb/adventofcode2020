...
char passwords[1000][100] = {"1-8 n: dpwpmhknmnlglhjtrbpx",
...
int get_cnt(char** password, int *offset)
{
int digit = 0;
*offset = 0;
char temp = **password;
...
char get_keychar(char** password, int* offset)
{
	char temp = **password;
	(*password)++;
	*offset = 1;
...
int main(void)
{
	...
	for (size_t i = 0; i<num_passwords; i++) {
		...
		int offset = 0;
		int least_cnt = get_cnt(&password, &offset);
		//password = password + offset;
		int max_cnt = get_cnt(&password, &offset);
		//password = password + offset;
		char keychar = get_keychar(&password, &offset);
        //password = password + offset;
		
		if( i == 0) { printf("%u-%u %c\n", least_cnt, max_cnt, keychar);}
		
		int count = 0;
		int k =0;
		while(password[k] != '\0') {
			if (password[k] == keychar) {++count;}
			k++;
		};
	    if ((count>= least_cnt) && (max_cnt >= count)) {valid_passwords++;}
	}
    printf("%u\n", valid_passwords);
	
	int valid_passwords_2 = 0;
	for (size_t index = 0; index<num_passwords; index++) {
		...
		password++;
		password++;
		if( (password[least_cnt - 1] == keychar)  ^ (password[max_cnt - 1] == keychar) ) {++valid_passwords_2;}
	}
    printf("%u\n", valid_passwords_2);
