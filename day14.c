...
#define STR_LEN                  50
#define BITMASK_OFFSET    7
#define MEM_ADDR_OFFSET 4

char  test[4][STR_LEN] ={"mask = XXXXXXXXXXXXXXXXXXXXXXXXXXXXX1XXXX0X",
"mem[8] = 11",
"mem[7] = 101",
"mem[8] = 0",};
char sample[580][STR_LEN] = {"mask = 0X10110X1001000X10X00X01000X01X01101",
"mem[49559] = 97",
"mem[18692] = 494387917",
"mem[9337] = 615452",
"mem[24695] = 3435491",
...
char test2[4][STR_LEN] = {"mask = 000000000000000000000000000000X1001X",
"mem[42] = 100",
...
typedef struct node_s {
 struct node_s* next;
 char bit_mask[BITMASK_SIZE];
  long unsigned int memory_addr;
 long unsigned int  value;
} node_t;

node_t* g_head_ptr = NULL;
node_t* g_tail_ptr = NULL;

long unsigned int  bit_masked_value(char* bit_mask_temp, long unsigned int  value)
{
	char bit_masked_out[BITMASK_SIZE] = {0};
	long unsigned int  bit_masked_value = value;
	long unsigned int bit_pos_mask =0;
	for (size_t i=0; i < BITMASK_SIZE; i++) {
		if (bit_mask_temp[BITMASK_SIZE - 1 - i] =='1') {
				//bit_masked_out[i] = '1';
            bit_pos_mask = (1LU<<i);
			bit_masked_value = (bit_masked_value | bit_pos_mask);
			//printf("\n bit_pos %lu,  OR with %lu result %lu", i, 1<<i, bit_masked_value);
		}
		else if (bit_mask_temp[BITMASK_SIZE - 1 - i] =='0') {
				//bit_masked_out[i] = '1';
            bit_pos_mask = ~(1LU<<i);
			bit_masked_value = (bit_masked_value & bit_pos_mask);
			//printf("\n bit_pos %lu,  AND with %lu  result %lu", i, (long unsigned int )~(1<<i),  bit_masked_value);
		}
	}
	char temp[BITMASK_SIZE+1] = {0};
	strncpy(temp, bit_mask_temp, BITMASK_SIZE);
	printf("\n mask %s input:%lu bit_masked_value %lu", temp, value, bit_masked_value);
	return bit_masked_value;
}

void memory_decoder(char* bit_mask_temp,  long unsigned int memory_addr, char* memort_bit_map, long unsigned int* num_x_bits)
{
	char bit_masked_out[BITMASK_SIZE] = {0};
	long unsigned int bit_pos_mask =0;
	for (size_t i=0; i < BITMASK_SIZE; i++) {
		if (bit_mask_temp[i] =='1') {
				bit_masked_out[i] = '1';
		}
		else if (bit_mask_temp[i] =='0') {
			bit_masked_out[i] = '0';
            bit_pos_mask = 1LU<<(BITMASK_SIZE - 1 -i);
			if (memory_addr & bit_pos_mask) {
				bit_masked_out[i] = '1';
			}
			else {
				bit_masked_out[i] = '0';
	...
	char temp1[BITMASK_SIZE+1] = {0};
	char temp2[BITMASK_SIZE+1] = {0};
	strncpy(temp1, bit_mask_temp, BITMASK_SIZE);
	strncpy(memort_bit_map, bit_masked_out, BITMASK_SIZE);	
	//printf("\n mask %s input:%lu bit_masked_value %s", temp1, memory_addr, memort_bit_map);

}


node_t* create_node(char* bit_mask_temp,  long unsigned int memory_addr, long unsigned int  value) 
{
	node_t* node = malloc(sizeof(node_t));
	node->next = NULL;
	strncpy(node->bit_mask, bit_mask_temp, BITMASK_SIZE);
	node->memory_addr = memory_addr;
	node->value = value;
	return node;
}

void read_store(char* data, int num_entries)
{
	char* data_org = data;
	char bit_mask_temp[BITMASK_SIZE];
	long unsigned int memory_addr  = 0;
	for (int i = 0; i < num_entries; i++) {
		data = data_org + i*STR_LEN;
		long unsigned int  value;
		int memory_value = 0;
		if (strstr(data, "mask")) {
			strncpy(bit_mask_temp, data+BITMASK_OFFSET, BITMASK_SIZE);
		}
		else {
			memory_value = 1;
			memory_addr = atoi(data+MEM_ADDR_OFFSET);
			char* temp = strstr(data, "=");
			temp++;
			temp++;
			value = atoi(temp);
			value = bit_masked_value(bit_mask_temp, value);
		}
		if (memory_value) {
			if (g_head_ptr == NULL) {
				node_t* node = create_node(bit_mask_temp, memory_addr, value);
					g_head_ptr = node;
			}
			else {
				node_t* node = g_head_ptr;
				int found = 0;
				while(node != NULL) {
					if(node->memory_addr == memory_addr) {
					    printf("\n FOUND memory_addr %lu previous value:%lu new value %lu", memory_addr, node->value , value);
						node->value =value;
						found = 1;
						break;
					}
...
long unsigned int  memory_sum(char* data, int num_entries) 
{
	g_head_ptr = NULL;
	read_store(data, num_entries);
...
void add_node(char* bit_mask_temp, long unsigned int memory_addr, long unsigned int value)
{
	if (g_head_ptr == NULL) {
		node_t* node = create_node(bit_mask_temp, memory_addr, value);
			g_head_ptr = node;
...
void read_store_2(char* data, int num_entries)
{
	char* data_org = data;
	char bit_mask_temp[BITMASK_SIZE];
	long unsigned int  memory_addr  = 0;
	...
	for (int i = 0; i < num_entries; i++) {
		...
		long unsigned int  value;
		int memory_value = 0;
		char memory_addr_bit_map[BITMASK_SIZE];
		long unsigned int  num_x_bits = 0;
		if (strstr(data, "mask")) {
			strncpy(bit_mask_temp, data+BITMASK_OFFSET, BITMASK_SIZE);
		}
		else {
			...
			temp++;
			value = atoi(temp);
			memory_decoder(bit_mask_temp, memory_addr, memory_addr_bit_map , &num_x_bits);
		}
		
		if (memory_value) {
		 long unsigned int num_address = (1LU<<num_x_bits);
		 for (size_t i = 0; i < num_address; i++) {
				char temp[BITMASK_SIZE];
				strncpy(temp, memory_addr_bit_map, BITMASK_SIZE);
				long unsigned int  num_bits_toggled = 0;
				memory_addr = 0;
				for ( size_t j = 0; j < BITMASK_SIZE; j++) {
					if (temp[BITMASK_SIZE - 1 - j] == 'X') {
						if (i & (1LU << num_bits_toggled)) {
							temp[BITMASK_SIZE - 1 - j] = '1';
							memory_addr += (1LU<<j);
						}
						else {
							temp[BITMASK_SIZE - 1 - j] = '0';
						}
						num_bits_toggled++;
					}
					else if (temp[BITMASK_SIZE - 1 - j] == '1') {
						memory_addr +=  (1LU<<j);
					}
				}
				char temp1[BITMASK_SIZE+1] = {0};
				strncpy(temp1, temp, BITMASK_SIZE);
	            //printf("\n i: %lu address %s memory address:%lu ", i, temp1, memory_addr);
				
				add_node(bit_mask_temp, memory_addr, value);
			}
		}
	}
}

long unsigned int  memory_sum_2(char* data, int num_entries) 
{
	g_head_ptr = NULL;
	read_store_2(data, num_entries);
...
int main(void)
{
	...
	printf(" \n Sum %lu \n", memory_sum((char*)test, 4));
	printf(" \n Sum %lu \n", memory_sum((char*)sample, 580));
	printf(" \n Sum %lu \n", memory_sum_2((char*)test2, 4));
	printf(" \n Sum %lu \n", memory_sum_2((char*)sample, 580));
return 0;
}