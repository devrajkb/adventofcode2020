...
#include <stdlib.h>
#include <ctype.h>
char test[13][300]= {"ecl:gry pid:860033327 eyr:2020 hcl:#fffffd",
...
char invalid_passport_value_test[13][300]={"eyr:1972 cid:100",
...
char valid_passport_value_test[12][300]= {"pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980",
"hcl:#623a2f",
"",
...
int valid_passorts_random_test(char* data, int lines, int max_length)
{
	//puts(*data);
	puts(data);
	...
	char keyword[4] ="";
	printf("length of line %lu", strlen(data));
	
	return 0;
}

void get_key_value(char* line, char* key, char* value, int *offset)
{
	//*offset = 0;
	int i = *offset;
	int key_len = 3;
	...
	*offset = i;
	//printf("line %s key %s, value %s offset %u\n", line, key, value, *offset);

}

void key_found(int *keys, char* key) {
	if (strcmp(key, "byr") == 0) {
		keys[KEY_BYR] = 1;
	}
...
void key_found_with_valid_value(int *keys, char* key,char* value) {
	if ((strcmp(key, "byr") == 0)) {
		if ((strlen(value) == 4)&& (atoi(value) >= 1920)  && (atoi(value) <= 2002))  {
			keys[KEY_BYR] = 1;
		}
	}
	else if (strcmp(key, "iyr") == 0) {
		if ((strlen(value) == 4)&& (atoi(value) >= 2010)  && (atoi(value) <= 2020))  {
			keys[KEY_IYR] = 1;
		}
	}
	else if (strcmp(key, "eyr") == 0) {
		if ((strlen(value) == 4)&& (atoi(value) >= 2020)  && (atoi(value) <= 2030))  {
			keys[KEY_EYR] = 1;
		}
	}
	else if (strcmp(key, "hgt") == 0) {
		if ((strstr(value, "in") && (atoi(value)>=59) && (atoi(value) <= 76))) {
			keys[KEY_HGT] = 1;
		}
		else if ((strstr(value, "cm") && (atoi(value)>=150) && (atoi(value) <= 193))) {
			keys[KEY_HGT] = 1;
		}
	}
   else if (strcmp(key, "hcl") == 0) {
	   if (strlen(value) == 7) {
			...
			int count = 0;
		     for (int ndx=0; ndx<6; ndx++) {
				if (isxdigit(value2[ndx]) && !isupper(value2[ndx])) {
					count++;
				}
			 }
			 if (count == 6) {
	   ...
	   if ((strlen(value) == 3) && ((strcmp(value, "amb") == 0) || (strcmp(value, "blu") == 0) || 
...
int required_keys_present(int *keys) {
	int found = 1;
	for ( size_t i = 0; i < KEY_CID; i++) {
		if(keys[i] == 0) {
			found = 0;
			break;			
		}
...
int valid_passorts(char* data, int lines, int max_length)
{

	int keys[KEY_LAST] ={0};
	int offset = 0;
    int valid_passorts_cnt = 0;
	...
	for ( size_t i = 0; i < lines; i++) {
		...
		while(line[offset] != '\0') {
			char value[20] = "";
		    char key[4] ="";
			get_key_value(line, key, value, &offset);
			key_found(keys, key);
		};
		
		if((offset == 0) || (i == lines - 1)) {
			if(required_keys_present(keys)) {
				valid_passorts_cnt++;
			//	printf("valid_passorts_cnt %u\n\n", valid_passorts_cnt);
			}
			memset(keys, 0, KEY_LAST*sizeof(int));
		}
		data = data + max_length;
	}
	printf("Count %u\n", valid_passorts_cnt);
	
	return valid_passorts_cnt;
}


int valid_passorts_key_value(char* data, int lines, int max_length)
{
	...
	for ( size_t i = 0; i < lines; i++) {
		...
		while(line[offset] != '\0') {
		    ...
		    char key[4] ="";
			get_key_value(line, key, value, &offset);
			key_found_with_valid_value(keys, key, value);
		};
...
int main(void)
{
    ...
    //valid_passorts_random_test((char*)test, 12, 300);
	printf("Valid passorts %u\n", valid_passorts((char*)test, 13, 300));
    printf("Valid passorts %u\n", valid_passorts((char*)sample, 953, 300));
	
    printf("Valid passorts with valid value %u\n", valid_passorts_key_value((char*)valid_passport_value_test, 12, 300));
    printf("Valid passorts with valid value  %u\n", valid_passorts_key_value((char*)invalid_passport_value_test, 13, 300));
	printf("Valid passorts with valid value  %u\n", valid_passorts_key_value((char*)sample, 953, 300));
